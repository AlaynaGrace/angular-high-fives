...
myApp.config(function($routeProvider){
  var templateToUse = "<img ng-src='{{highFiveBuddy.imageLink}}'>"+
   ...
   "<p>Barry's high five skill level: {{highFiveBuddy.skillLevel}}</p>" +
   "<p>Click the button below to {{highFiveBuddy.name}} high five!</p>" +
   "<button class='btn' ng-click='hf.highFiveMe()'><img src='https://www.spreadshirt.com/image-server/v1/designs/12910541,width=178,height=178/high-five-hand-sign-high-5-hand-gesture-sign-langu.png'></button>" +
   "<h3>You've made {{hf.successful}} out of {{hf.total}} high fives</h3>";

   $routeProvider.when('/barry',{
 ...
 myApp.controller('Barry', function($scope, $route, GetHighFives){
   $scope.highFiveBuddy = {
     ...
     skillLevel: GetHighFives.getSkillLevel(),
     name: 'Barry'
   };
 });
